...
st.caption(f"系统时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 核心模式: 谷歌探测(7D) + 腾讯实时行情")

# -----------------------------
# 2️⃣ 2026 核心数据字典
# -----------------------------
SECTOR_CONFIG = {
    "机器人/智造": {"keywords": "(机器人 OR 行星丝杠 OR 灵巧手 OR 减速器 OR 特斯拉机器人)", "stocks": ["603728", "300024", "002031"]},
...
def analyze_hot_keywords(df):
    if df.empty: return []
    # 过滤掉非实质性高频词
    stop_words = ["财经", "新闻", "发布", "公司", "中国", "市场", "披露", "进行", "分析", "关注"]
...
if probe_trigger and manual_key:
    ...
    if not news.empty:
        ...
        for _, row in news.iterrows():
            with st.expander(f"[{row['source']}] {row['title']}"):
                st.caption(f"时间: {row['time']}")
                st.link_button("查看全文", row['link'])
    else:
        st.warning("未能穿透相关线索，请尝试简化关键词（如：机器人）。")
    ...
    with col2:
        ...
        if not sector_news.empty:
            ...
            st.warning("⚠️ 探测受阻。请检查全局网络，或更换行业词库。")
...
st.caption("Nova 审计脚注：已彻底移除新浪接口，全面采用腾讯 Qt 与 Google 7D 镜像穿透技术。")
